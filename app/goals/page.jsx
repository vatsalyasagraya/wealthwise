...
export default function GoalsPage() {
  ...
  const handleCreateGoal = async () => {
    ...
    const { error } = await supabase.from('goals').insert({
      user_id: user.id, name, target_amount: parseFloat(targetAmount), saved_amount: 0, emoji,
    })
    if (error) { setMessage(error.message) }
    else { setName(''); setTargetAmount(''); setEmoji('🎯'); setShowForm(false); fetchGoals() }
    setSaving(false)
  }
  ...
  const handleAddMoney = async (goal) => {
    ...
    const { error } = await supabase.from('goals').update({ saved_amount: newSaved }).eq('id', goal.id)
    if (!error) { setAddingTo(null); setAddAmount(''); fetchGoals() }
  }
      ...
      {showForm && (
        <div className="bg-white rounded-2xl shadow-sm p-6 space-y-4">
            ...
            <div className="flex gap-2 mt-2 flex-wrap">
              {EMOJI_OPTIONS.map((e) => (
                <button key={e} onClick={() => setEmoji(e)}
                  className={`text-xl p-2 rounded-lg transition-all ${emoji === e ? 'bg-indigo-100 ring-2 ring-indigo-400' : 'bg-gray-100 hover:bg-gray-200'}`}
                >
                  {e}
                </button>
              ))}
          ...
          <div>
            ...
            <input type="text" placeholder="e.g. Trip to Goa, New Laptop"
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-400">
                        {isComplete ? '✅ Goal reached!' : `₹${remaining.toLocaleString('en-IN')} remaining`}
                      </p>
                    </div>
                  </div>
                  <button onClick={() => handleDelete(goal.id)} className="text-red-400 hover:text-red-600 text-xs">✕</button>
                </div>
                ...
                <div className="flex items-center gap-3 mb-4">
                  <div className="relative w-16 h-16 flex-shrink-0">
                    <svg viewBox="0 0 36 36" className="w-16 h-16 -rotate-90">
                      ...
                      <circle cx="18" cy="18" r="15.9" fill="none" stroke={isComplete ? '#22c55e' : '#6366f1'}
                  ...
                  <div className="flex-1">
                    <div className="w-full bg-gray-100 rounded-full h-2 mb-2">
                      <div className={`h-2 rounded-full transition-all duration-500 ${isComplete ? 'bg-green-500' : 'bg-indigo-500'}`}
                        style={{ width: `${percent}%` }} />
                    </div>
                ...
                {!isComplete && (
                  addingTo === goal.id ? (
                    <div className="flex flex-col sm:flex-row gap-2">
                      ...
                      />
                      <button onClick={() => handleAddMoney(goal)} className="bg-indigo-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-indigo-700 transition-all">Add</button>
                      <button onClick={() => { setAddingTo(null); setAddAmount('') }} className="bg-gray-100 text-gray-500 px-4 py-2 rounded-lg text-sm hover:bg-gray-200 transition-all">Cancel</button>
                    </div>
                  ) : (
