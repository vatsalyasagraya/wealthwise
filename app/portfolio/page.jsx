...
export default function PortfolioPage() {
  ...
  const byType = investments.reduce((acc, inv) => { acc[inv.type] = (acc[inv.type] || 0) + inv.amount; return acc }, {})
      ...
      <div className="max-w-3xl mx-auto px-6 py-8 space-y-6">
        ...
        <div className="flex justify-between items-start gap-1">
          <div>
            ...
            <p className="text-gray-500 text-sm mt-1">Track all your investments in one place</p>
          </div>
          <button
        ...
        {showForm && (
          <div className="bg-white rounded-2xl shadow-sm p-6 space-y-4">
            ...
            <div>
              <label className="text-sm font-medium text-gray-700">Investment Name</label>
              <input
                type="text" placeholder="e.g. Reliance Industries, Nifty 50 Index Fund"
            ...
            <button
              onClick={handleAddInvestment} disabled={saving}
        ...
        <div className="bg-gradient-to-r from-indigo-600 to-purple-600 rounded-2xl p-6 text-white">
          ...
          <h2 className="text-3xl font-bold mt-1">₹{total.toLocaleString('en-IN')}</h2>
          <p className="text-indigo-200 text-sm mt-1">{investments.length} investments tracked</p>
        </div>
            ...
            <ResponsiveContainer width="100%" height={260}>
              <PieChart>
                <Pie data={chartData} cx="50%" cy="50%" innerRadius={70} outerRadius={110} paddingAngle={4} dataKey="value">
                  {chartData.map((entry) => (
                    <Cell key={entry.name} fill={TYPE_CHART_COLORS[entry.name] || '#94a3b8'} />
                  ))}
                </Pie>
            ...
            <div className="flex flex-wrap justify-center gap-x-4 gap-y-2 mt-4">
              {chartData.map((entry) => (
                <div key={entry.name} className="flex items-center gap-2">
                  <div className="w-3 h-3 rounded-full flex-shrink-0" style={{ background: TYPE_CHART_COLORS[entry.name] || '#94a3b8' }} />
                  <span className="text-xs text-gray-600">{entry.name}</span>
                  <span className="text-xs font-semibold text-gray-800">{((entry.value / total) * 100).toFixed(1)}%</span>
                </div>
              ))}
            </div>
            <p className="text-center text-sm text-gray-400 mt-3">Total: ₹{total.toLocaleString('en-IN')}</p>
          </div>
        )}
        ...
        <div className="bg-white rounded-2xl shadow-sm p-6">
          ...
          {loading ? (
            ...
            <p className="text-gray-400 text-sm text-center py-8">No investments yet. Add your first one! 👆</p>
          ) : (
            <div className="space-y-3">
              {investments.map((inv) => (
                <div key={inv.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-xl">
                  <div className="flex items-center gap-3">
                    <span className={`text-xs font-medium px-2 py-1 rounded-full ${TYPE_COLORS[inv.type]}`}>{inv.type}</span>
                    <span className="text-sm font-medium text-gray-800">{inv.name}</span>
                  </div>
                  <div className="flex items-center gap-4">
                    <span className="text-sm font-semibold text-gray-800">₹{inv.amount.toLocaleString('en-IN')}</span>
                    <button onClick={() => handleDelete(inv.id)} className="text-red-400 hover:text-red-600 text-xs">✕</button>
                  </div>
                </div>
              ))}
            </div>
