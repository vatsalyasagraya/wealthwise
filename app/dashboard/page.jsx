...
export default function DashboardPage() {
  ...
  // useUser will redirect to /login — this null prevents a flash of content
  if (!user) return null
            ...
            {portfolio.length > 0 && (
              <ResponsiveContainer width="100%" height={200}>
                <BarChart data={portfolio} margin={{ top: 0, right: 0, left: 0, bottom: 0 }}>
                  ...
                  <Tooltip formatter={(value) => [`₹${value.toLocaleString('en-IN')}`, 'Value']} cursor={{ fill: '#f1f5f9' }} />
        ...
        <div className="space-y-5">
          {goals.length === 0 ? (
            <p className="text-gray-400 text-sm text-center py-4">
              No goals yet.{' '}
              <button onClick={() => router.push('/goals')} className="text-indigo-500 ml-1 hover:underline">
                Create one →
              </button>
            </p>
          ) : (
