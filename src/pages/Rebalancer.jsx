...
export default function Rebalancer({ user }) {
  ...
  // Calculate actual % per type
  const actual = INVESTMENT_TYPES.reduce((acc, type) => {
    acc[type] = totalPortfolio > 0
      ? ((byType[type] || 0) / totalPortfolio) * 100
      : 0
    return acc
  }, {})
  ...
  // Update a target slider — also keep total in sync
          ...
          {isTotalValid && (
            <div className="bg-white rounded-2xl shadow-sm p-6">
              ...
              <div className="space-y-3">
                {suggestions.map((s) => {
                  const isOnTarget = Math.abs(s.diff) < 500 // within ₹500 = fine
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-gray-400">
                            {s.actualPercent.toFixed(1)}% actual → {s.targetPercent}% target
                          </p>
                        </div>
                      </div>
                      <div className="text-right">
                        {isOnTarget ? (
                          ...
                          <>
                            <span className={`text-sm font-bold ${isBuy ? 'text-green-600' : 'text-red-500'}`}>
                              {isBuy ? '▲ BUY' : '▼ SELL'}
                            </span>
                            <p className="text-xs text-gray-500">
                              ₹{Math.abs(s.diff).toLocaleString('en-IN')}
                            </p>
                          </>
                        )}
                      </div>
              ...
              <p className="text-xs text-gray-400 mt-4 text-center">
                * Suggestions are based on your current portfolio value of ₹{totalPortfolio.toLocaleString('en-IN')}
              </p>
            </div>
          )}
        </>
